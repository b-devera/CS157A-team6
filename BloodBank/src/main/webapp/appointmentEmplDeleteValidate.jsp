...
<%
	...
	if(!(apptID.equals(null) || apptID.equals("")))
	{
		...
		try{
			...
			if(result == 1){
				getServletContext().getRequestDispatcher("/appointmentEmployee.jsp").include(request,response);
				%>
			...
			<%
				getServletContext().getRequestDispatcher("/appointmentEmployeeDelete.jsp").include(request,response);
		} 
	}
	else{
		%>
